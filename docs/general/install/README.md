...
```hcl
module "app-mesh" {
    source = "github.com/pbs/terraform-app-mesh-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/app-mesh'
gh -R 'pbs/terraform-app-mesh-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/app-mesh'
rm -f release.tar.gz
```
...
```bash
git remote add -f app-mesh git@github.com:pbs/terraform-app-mesh-module.git
git subtree add --prefix terraform/modules/app-mesh app-mesh main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/app-mesh app-mesh $REF --squash
```
